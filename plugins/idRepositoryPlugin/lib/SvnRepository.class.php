...
class SvnRepository {
	...
	private function executeShellCommand()
  {
    ...
    if($error){
      ...
      if (file_exists($this->svn_xml_reader->getFilePath()))
      {
        $error .= " [".file_get_contents($this->svn_xml_reader->getFilePath())."]";
      }

      throw new Exception('Shell Command Error when executing '.$this->proxy->getCommand().' [error :'.$error.'][output : '.print_r($output).']');
		}
	}
