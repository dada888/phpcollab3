...
class idDoctrineTestFunctional extends sfTestFunctional
{
  ...
  public function inizilizeDatabase()
  {
    ...
    $command = "sf12 doctrine:drop-db --env=test --no-confirmation; ";
    $command .= "sf12 doctrine:build-db --env=test; ";
    $command .= "sf12 doctrine:build-sql --env=test; ";
    $command .= "sf12 doctrine:insert-sql --env=test; ";
    $command .= "sf12 doctrine:data-load --env=test --dir=test/fixtures/fixtures.yml";
