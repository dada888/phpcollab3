          ...
          <?php else: ?>
            <?php foreach ($priority_list as $priority): ?>
              <tr class="odd">
                ...
                <td><?php echo link_to(__('Edit'), '@edit_priority?id='.$priority->getId()) ?> | <?php echo link_to(__('Delete'), '@delete_priority?id='.$priority->getId(), array('confirm' => __('Do you really want to delete this priority?'))) ?></td>
                <td>&nbsp;</td>
              </tr>
            <?php endforeach; ?>
          <?php endif; ?>
