      ...
      <form action="<?php echo url_for('@index_project') ?>" method="get" class="form">
        <table class="table">
          ...
          <tr class="odd">
            ...
            <td><?php echo /*input_date_tag('created_at', date('Y-m-d', time()), 'rich=true');*/$form['created_at']->renderError('<br/>') ?><?php echo $form['created_at'] ?></td>
            <td>&nbsp;</td>
          </tr>
        </table>
        <div class="actions-bar">
          <div class="actions">
            <?php echo link_to('Reset', '@index_project') ?> or <input type="submit" class="button" value="<?php echo __('Filter') ?>" />
          </div>
          <div class="clear"></div>
        </div>
      </form>
